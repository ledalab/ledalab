...
p.addParameter('zscor', 1, @isnumeric)
...
%             TODO: USE allcomb() HERE??!!
    ...
    for i = 1:max(reps)
        ...
        sbf_rename_exp(fcell, ['_era' zstr]...
            , Arg.era_beg(i), Arg.era_end(i), Arg.scr_thr(i), Arg.sav_typ)
    end
else
    Ledalab(fcell...
...
if ~isscalar(scrlist_thr)
    for i = 1:numel(scrlist_thr)
        ...
        sbf_rename_exp(fcell, ['_scrlist' zstr], '', '', scrlist_thr(i), Arg.sav_typ)
    end
else
    Ledalab(fcell...
        , 'open', 'leda'...
    ...
    sbf_rename_exp(fcell, ['_scrlist' zstr], '', '', scrlist_thr, Arg.sav_typ)
end
end
